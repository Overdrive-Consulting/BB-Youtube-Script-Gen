...
const Dashboard: React.FC = () => {
  ...
  const sendToWebhook = async (idea: ScriptIdea) => {
    ...
    try {
      ...
      const payload = {
        ...
        target_audiences: data.target_audiences || '' // Include target_audiences in the payload
      };
  ...
  const handleGenerateScript = async (idea: ScriptIdea, event: React.MouseEvent) => {
    ...
    toast.promise((async () => {
      ...
      // Only update the status to "Content Generated", no dummy data
      await updateScriptIdea.mutateAsync({
        id: idea.id,
        status: 'Content Generated' // Only update the status
      });
      return 'Script request sent successfully. The script will be generated by the workflow.';
    })(), {
